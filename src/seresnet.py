...
def main():
    ...
    train_dataset = MyDataset(file_list="../SplitAndIngreLabel/TR.txt", data_root="./ready_chinese_food",
    ...
    validate_dataset = MyDataset(file_list="../SplitAndIngreLabel/VAL.txt", data_root="./ready_chinese_food",
                                 transform_type="val")
    ...
    #for param in net.layer1.parameters():  # 解冻layer4层
...
def mytest():
    ...
    nw = min([os.cpu_count(), batch_size if batch_size > 1 else 0, 8])  # number of workers
    device = torch.device("cuda:0" if torch.cuda.is_available() else "cpu")
    test_dataset = MyDataset(file_list="../SplitAndIngreLabel/TE.txt", data_root="./ready_chinese_food",
                             transform_type="val")
    ...
    net = resnet101(num_classes=172, include_top=True)
    # load pretrain weights
    model_weight_path = "../weights/resnet101.pth"
    assert os.path.exists(model_weight_path), "file {} does not exist.".format(model_weight_path)
    net.load_state_dict(torch.load(model_weight_path, map_location='cpu'))
